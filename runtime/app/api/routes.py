...
def chat(aid:str,body:AgentPrompt,db:Session=Depends(get_session)):
 a=db.get(AgentRecord,aid)
 if not a:raise HTTPException(404,'Agent not found')
 try:return AgentReply(agent_id=a.id,text=run_agent(a,body.prompt))
 except Exception as e:raise HTTPException(502,'Provider error: '+str(e))
def view(r):return AgentView(id=r.id,project_id=r.project_id,name=r.name,role=r.role,state=r.state,llm=LLMConfigInput(provider_id=r.provider_id,connection_id=r.connection_id,model=r.model,endpoint=r.endpoint,context_limit=r.context_limit,temperature=r.temperature_milli/1000,cloud_fallback_allowed=r.cloud_fallback_allowed))
